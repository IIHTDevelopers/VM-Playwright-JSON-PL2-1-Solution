...
test.describe("Yaksha", () => {
  ...
  test("TS-10 Verify Nursing Check-in for an Outpatient", async ({ page }) => {
    await nursingPage.verifyNursingCheckinForOutPatient();
...
async function verifyTableVisibility(page: Page) {
  const table = page.locator("//div[normalize-space()='JV-1']/../..");
...
async function verifyBedStatusUpdated(page: Page) {
  const successMessage = await page
    .locator('[class="main-message"]')
    .textContent();
  expect(successMessage).toBe("Bed Feature Updated");
}

async function verifyTriageUpdated(page: Page) {
  const successMessage = await page
    .locator('(//p[@class="msg-status"]/../p)[2]')
    .textContent();
  expect(successMessage).toBe("Chief Complaint Updated Successfully");
}
